...
import net.whatamidoingstudios.lacroix.network.HeaterMessageServer;
...
public class GuiHeater extends GuiContainer {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(mouseX >= guiLeft+140 && mouseX < guiLeft+160) {
			if(mouseY >= guiTop+20 && mouseY < guiTop+73) {
				drawHoveringText(steam+"/"+maxSteam+" steam", mouseX, mouseY);
			}
		}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		fontRenderer.drawString(typename + " " + name, xSize / 2 - (fontRenderer.getStringWidth(typename+" "+name)) / 2, 6, 0x404040);
